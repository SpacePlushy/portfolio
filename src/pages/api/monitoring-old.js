/**
 * Backup of the original monitoring endpoint
 */

import { getSharpStatus } from '../../utils/sharp-loader.js';
import { createClient } from 'redis';

// Metrics storage with automatic cleanup
const MAX_REQUEST_ENTRIES = 1000;
...
function cleanupMetrics() {
  ...
  if (now - metrics.lastCleanup < CLEANUP_INTERVAL) {
    return;
  }
  
  metrics.lastCleanup = now;
  
  // Cleanup requests Map - keep only most frequently accessed entries
  if (metrics.requests.size > MAX_REQUEST_ENTRIES) {
    const entries = Array.from(metrics.requests.entries())
      .sort((a, b) => b[1].count - a[1].count)
      .slice(0, MAX_REQUEST_ENTRIES);
    
    metrics.requests.clear();
    entries.forEach(([key, value]) => metrics.requests.set(key, value));
  }
  
  // Cleanup old performance entries (keep last 30 minutes)
  const thirtyMinutesAgo = now - (30 * 60 * 1000);
  metrics.performance = metrics.performance.filter(p => p.timestamp > thirtyMinutesAgo);
  
  // Cleanup old error entries (keep last 1 hour)
  const oneHourAgo = now - (60 * 60 * 1000);
  metrics.errors = metrics.errors.filter(e => e.timestamp > oneHourAgo);
  
  // Cleanup old memory snapshots (keep last 30 minutes)
  metrics.memory.snapshots = metrics.memory.snapshots
...
function recordRequest(data) {
  ...
  // Keep only last MAX_PERFORMANCE_ENTRIES entries
  if (metrics.performance.length > MAX_PERFORMANCE_ENTRIES) {
    metrics.performance.splice(0, metrics.performance.length - MAX_PERFORMANCE_ENTRIES);
  }
}
...
function recordError(data) {
  ...
  // Keep only last MAX_ERROR_ENTRIES errors
  if (metrics.errors.length > MAX_ERROR_ENTRIES) {
    metrics.errors.splice(0, metrics.errors.length - MAX_ERROR_ENTRIES);
  }
}
...
export async function GET({ url }) {
  ...
  // Record memory snapshot with size limit
  ...
  if (metrics.memory.snapshots.length > MAX_MEMORY_SNAPSHOTS) {
    metrics.memory.snapshots.splice(0, metrics.memory.snapshots.length - MAX_MEMORY_SNAPSHOTS);
  }
